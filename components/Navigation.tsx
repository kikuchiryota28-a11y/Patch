...
export function Navigation(){
 ...
 return <>
  <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] border-r border-white/[0.07] bg-zinc-950/95 px-5 py-6 backdrop-blur-2xl md:flex md:flex-col">
   ...
   <div className="mt-auto space-y-3"><Link href={user?'/settings':'/login'} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-bold text-white transition hover:bg-white/[0.08]"><LogIn size={16}/>{user?t('nav.account'):t('nav.signIn')}</Link><button type="button" onClick={postIssue} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-4 py-4 text-sm font-black text-zinc-950"><Plus size={18}/>{t('nav.post')}</button><div className="flex items-center justify-between rounded-2xl border border-white/[0.07] bg-white/[0.025] p-2"><span className="pl-2 text-[11px] font-bold text-zinc-600">{t('nav.notifications')}</span><NotificationCenter/></div></div>
  </aside>
  <BottomNav onPost={postIssue}/>
