...
function isPatchStyle(value: string): value is PatchStyle {
  return patchStyles.includes(value as PatchStyle);
}
...
function mapIssue(row: IssueRow, patches: PatchRow[]): Issue {
  ...
  const title = firstLine?.trim() || 'Untitled Issue';
  ...
  return {
    ...
    patches: patches
      .filter((patch) => patch.issue_id === row.id)
      .sort((a, b) => b.upvotes - a.upvotes || Date.parse(b.created_at) - Date.parse(a.created_at))
      .map(mapPatch),
  };
}
...
export async function fetchIssues(): Promise<Issue[]> {
  ...
  if (!supabase) return initialIssues;

  const [{ data: issueRows, error: issueError }, { data: patchRows, error: patchError }] =
    await Promise.all([
      supabase
        .from('issues')
      ...
      supabase
        .from('patches')
        .select('id, issue_id, patched_text, patch_type, upvotes, created_at')
...
export async function createPatch(input: CreatePatchInput): Promise<PatchItem> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      patched_text: input.text.trim(),
      patch_type: input.style,
      upvotes: 0,
    })
    .select('id, issue_id, patched_text, patch_type, upvotes, created_at')
    .single();
  ...
  if (error) throw error;
  return mapPatch(data as PatchRow);
}

export async function upvotePatch(patchId: string): Promise<PatchItem> {
  const supabase = getSupabaseClient();
  if (!supabase) {
    throw new Error('Supabase is not configured. Local voting is handled in the UI state.');
  }
  ...
  if (error) throw error;
  if (!data) throw new Error('Patch was not found.');
