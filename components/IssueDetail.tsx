...
export function IssueDetail({id}:{id:string}){
  const{getIssue,upvotePatch,mergePatch,dbEnabled}=useIssues(); const issue=getIssue(id); const[open,setOpen]=useState(false); const[shareLabel,setShareLabel]=useState('Share to X'); const[actorId,setActorId]=useState('');
  ...
  return <main className="min-h-screen"><Header/><div className="mx-auto max-w-4xl px-4 pb-24 pt-8 md:px-6 md:pt-12">
    ...
    <section className="mt-10"><div className="mb-5 flex items-end justify-between gap-4"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-300/80">The Patch stack</div><h2 className="mt-1 text-xl font-bold">How the internet would say it</h2></div>{dbEnabled&&<span className="text-[11px] font-semibold text-emerald-300/80">Live database</span>}</div><div className="space-y-8">{currentIssue.patches.length?currentIssue.patches.map((p)=><PatchCard key={p.id} patch={p} before={currentIssue.body} onUpvote={()=>upvotePatch(currentIssue.id,p.id)} canMerge={Boolean(actorId)&&currentIssue.ownerActorId===actorId} onMerge={()=>mergePatch(currentIssue.id,p.id)}/>):<div className="rounded-3xl border border-dashed border-white/10 p-10 text-center text-sm text-zinc-500">No patches yet. Be the first.</div>}</div></section>
