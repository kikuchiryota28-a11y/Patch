...
export function AuthProvider({children}:{children:React.ReactNode}){const supabase=getSupabaseClient();const[user,setUser]=useState<User|null>(null);const[profile,setProfile]=useState<Profile|null>(null);const[loading,setLoading]=useState(true);const[authOpen,setAuthOpen]=useState(false);
async function refreshProfile(){if(!supabase||!user){setProfile(null);return}const{data}=await supabase.from('profiles').select('username,display_name,bio,avatar_url,language').eq('actor_id',user.id).maybeSingle();if(data)setProfile({username:data.username??'',displayName:data.display_name??'',bio:data.bio??'',avatarUrl:data.avatar_url??'',language:(String(data.language||'EN').toLowerCase() as Locale)});else setProfile(null)}
...
async function saveProfile(input:Profile){await completeProfile(input);setProfile(input);await refreshProfile()};const value=useMemo(()=>({user,profile,loading,authOpen,openAuth:()=>setAuthOpen(true),closeAuth:()=>setAuthOpen(false),profileComplete:Boolean(profile?.username&&profile?.displayName),needsOnboarding:Boolean(user&&!loading&&!profile),refreshProfile,saveProfile}),[user,profile,loading,authOpen]);return <C.Provider value={value}>{children}</C.Provider>}
